...
<div id="minicart" class="card">
  ...
  <#if (shoppingCartSize > 0)>
    <#if hidetoplinks?default("N") != "Y">
      <ul>
        <li>
          <a href="<@ofbizUrl>view/showcart</@ofbizUrl>" class="btn btn-outline-secondary" aria-label="${uiLabelMap.OrderViewCart}">
            <img src="/ecommerce/images/cart.svg" class="cart-icon-img" alt="${uiLabelMap.OrderViewCart}" />
            <span class="sr-only">${uiLabelMap.OrderViewCart}</span>
          </a>
        </li>
  ...
  <#else>
    <#-- Empty cart: no text to keep UI cleaner on small widgets -->
  </#if>
  </div>
</div>
